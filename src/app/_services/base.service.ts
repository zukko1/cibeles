...
export abstract class ServiceBase<M> {
  ...
  public GetList() {
    ...
    Params = Params.append('itemsPerPage', '10');
   ...
   * En el caso de los usuarios es el 'username' en los demás casos es el id númerico clasico
  ...
  public GetEntityById(id: string) {
    ...
    const url = this.PathBase + this.getModel() + '/' + id;
  ...
  public Save(entity: M) {
    ...
    const url = this.PathBase + this.getModel();
    const params: string = JSON.stringify(entity);
  ...
  public getCollection(url) {
    return this.http.get<M>(
      environment.baseIP + url,
    );
  }
